      ...
      <section className="py-32 px-6 bg-zinc-950/50">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            >
              <h2 className="text-gold font-serif text-4xl md:text-6xl mb-8">أحدث التقنيات الطبية</h2>
              <p className="text-stone-400 text-xl font-arabic leading-relaxed mb-10">
                نحن نؤمن بأن الدقة هي مفتاح الرؤية المثالية. لذلك، قمنا بتجهيز عيادتنا بأحدث الأجهزة الرقمية العالمية لفحص النظر وقياس تحدب القرنية، مما يضمن لك أدق النتائج وأفضل الحلول البصرية.
              </p>
              <div className="grid grid-cols-2 gap-6">
                {[
                  { label: "فحص رقمي متطور", icon: "💻" },
                  { label: "قياس دقيق للعدسات", icon: "📐" },
                  { label: "تشخيص مبكر", icon: "🔍" },
                  { label: "دقة متناهية", icon: "🎯" }
                ].map((tech, i) => (
                  <div key={i} className="flex items-center justify-end gap-4 p-4 rounded-2xl bg-black/40 border border-gold/10">
                    <span className="text-stone-300 font-arabic">{tech.label}</span>
            ...
            <motion.div 
              ...
              className="relative"
            >
              <div className="aspect-video rounded-[40px] overflow-hidden luxury-border">
                <img 
              ...
              <div className="absolute -bottom-8 -left-8 w-48 h-48 bg-gold/10 blur-[100px] rounded-full" />
            </motion.div>
          </div>
        </div>
      </section>
